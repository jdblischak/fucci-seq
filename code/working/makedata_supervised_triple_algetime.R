...
#  nvalid <- round(ncol(log2cpm.quant)*.15)
  ...
  saveRDS(data_training, file=paste0("data/results/triple_ind_",ind,"_data_training_alge.rds"))
  saveRDS(data_withheld, file=paste0("data/results/triple_ind_",ind,"_data_withheld_alge.rds"))
}
...
saveRDS(data_training, file="data/results/triple_data_training_alge.rds")
saveRDS(data_withheld, file="data/results/triple_data_withheld_alge.rds")
...
library(circular)
theta <- as.numeric(coord2rad(pca_triple$x[,1:2]))
names(theta) <- rownames(pdata)

set.seed(17)
theta_permute <- sample(theta)
...
saveRDS(data_permute_training, file="data/results/triple_data_permute_training.rds")
saveRDS(data_permute_withheld, file="data/results/triple_data_permute_withheld.rds")
...
for (ind in unique(pdata$chip_id)) {
  ...
  theta.nonvalid <- theta_permute[ii.nonvalid]
  theta.valid <- theta_permute[ii.valid]
  ...
  saveRDS(data_training, file=paste0("data/results/triple_ind_",ind,"_data_permute_training.rds"))
  saveRDS(data_withheld, file=paste0("data/results/triple_ind_",ind,"_data_permute_withheld.rds"))
}
