...
# Utility script to export counts for human and ERCC genes to gzipped
# tab-delimited file.
#
# Usage:
#
...
mat <- exprs(eset)
colnames(mat) <- paste(eset$chip_id, eset$experiment, eset$well, sep = ".")

# Make rownames of gene IDs a separate column to make it easier to read with
# data.table::fread.
mat_df <- data.frame(gene = rownames(mat), mat)
