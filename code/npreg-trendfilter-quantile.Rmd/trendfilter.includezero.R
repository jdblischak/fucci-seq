...
fit.trend.includezero <- mclapply(1:nrow(log2cpm.all), function(g) {
  fit.trendfilter.includezero(log2cpm.all[g,],
                              polyorder = 2)
}, mc.cores=10)

saveRDS(fit.trend.includezero,
        file.path(dir,"output/npreg-trendfilter-quantile.Rmd/fit.trend.includezero.rds"))
