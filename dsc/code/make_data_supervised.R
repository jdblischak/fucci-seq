#' @title prepare data partitions for supervised model
#'
#' @import Biobase
#'
#' @export
make_data_supervised <- function(partitions,
                                 pdata, fdata, log2cpm, log2cpm_normed, theta) {

  if (partitions$type == "individual_heldout") {

    inds <- unique(pdata$chip_id)
    for (j in 1:length(inds)) {
      ind <- inds[j]
      ii.valid <- partitions[[which_ind]]$valid
      ii.train <- partitions[[which_ind]][[1]]

      which_ind <- which(names(partitions) == ind)
      pdata.train <- pdata[partitions[[which_ind]]$,]
      pdata.valid <- pdata[ii.valid,]

      log2cpm_normed.train <- log2cpm_normed[,ii.nonvalid]
