...
Results show that there are some indication of cell cycle signal in teh data. Of the genes with s-vaule < .01, 33 correlate with DAPI are annotated as cell cycle gene, 74 correlation with RFP and annotated as cell cycle gene, and 110 correlate with GFP are annotated as cell cycle gene. 
...
genes.go.detect <- readRDS("../output/seqdata-select-cellcyclegenes.Rmd/genes.go.detect.rds")
genes.go <- readRDS(file = "../output/seqdata-select-cellcyclegenes.Rmd/genes.go.rds")
```
...
Of the ones with s-value < .01, consider the proportion of genes in our annotated set.

```{r}
dapi.sig <- corr.dapi.shrink$model$result$svalue < .01
go.genes <- names(corr.dapi.shrink$estimate) %in% genes.go
table(dapi.sig, go.genes)

rfp.sig <- corr.rfp.shrink$model$result$svalue < .01
go.genes <- names(corr.rfp.shrink$estimate) %in% genes.go
table(rfp.sig, go.genes)

gfp.sig <- corr.gfp.shrink$model$result$svalue < .01
go.genes <- names(corr.gfp.shrink$estimate) %in% genes.go
table(gfp.sig, go.genes)
```


Given that genes belong to GO cell cycle category, the proportion that has significant correlation with RFP, GFP, and DAPI intensities

DAPI

```{r}
sum(dapi.sig*go.genes)/sum(go.genes)
```

GFP

```{r}
sum(gfp.sig*go.genes)/sum(go.genes)
```

RFP

```{r}
sum(rfp.sig*go.genes)/sum(go.genes)
```


Of the ones with s-value < .01, consider the proportion of genes in our annotated set and also with detection rate > 50%

```{r}
dapi.sig <- corr.dapi.shrink$model$result$svalue < .01
go.genes <- names(corr.dapi.shrink$estimate) %in% genes.go.detect
table(dapi.sig, go.genes)

rfp.sig <- corr.rfp.shrink$model$result$svalue < .01
go.genes <- names(corr.rfp.shrink$estimate) %in% genes.go.detect
table(rfp.sig, go.genes)

gfp.sig <- corr.gfp.shrink$model$result$svalue < .01
go.genes <- names(corr.gfp.shrink$estimate) %in% genes.go.detect
table(gfp.sig, go.genes)
```


Given that genes belong to GO cell cycle category and with detection rate > 50%, the proportion that has significant correlation with RFP, GFP, and DAPI intensities
...
save genes that are either correlated with DAPI, RFP or GFP

```{r}
go.genes <- names(corr.dapi.shrink$estimate)[names(corr.dapi.shrink$estimate) %in% genes.go]

dapi.sig <- names(corr.dapi.shrink$estimate)[corr.dapi.shrink$model$result$svalue < .01]
rfp.sig <- names(corr.dapi.shrink$estimate)[corr.rfp.shrink$model$result$svalue < .01]
gfp.sig <- names(corr.dapi.shrink$estimate)[corr.gfp.shrink$model$result$svalue < .01]
sig.union <- unique(c(dapi.sig, rfp.sig, gfp.sig))

genes.corr <- sig.union[sig.union %in% go.genes]
```

```{r}
saveRDS(genes.corr, file = "../output/images-seq-correlation.Rmd/genes.corr.rds")
```
