...
saveRDS(fits.trend2, file = "../output/method-train-classifiers.Rmd/fits.trend2.rds")
```

load results

```{r}
fits.nw <- readRDS(file = "../output/method-train-classifiers.Rmd/fits.nw.rds")
fits.trend2 <- readRDS(file = "../output/method-train-classifiers.Rmd/fits.trend2.rds")
```
...
save(nw, trend2, 
     file="../output/method-train-classifiers.Rmd/pve.methods.rda")
```

```{r}
load(file="../output/method-train-classifiers.Rmd/pve.methods.rda")
...
## Unsupervsied methods fitted across then extract test samples

the average prediction error across the test samples way higher than for all. this suggest that we should fit for each test sample at a time
...
fit.trend2.unsup$diff_time <- with(fit.trend2.unsup,
          pmin(abs(cell_times_est_shift-ref_time),
               abs(cell_times_est_shift-(2*pi-ref_time))))
saveRDS(fit.trend2.unsup, "../output/method-train-classifiers.Rmd/fit.trend2.unsup.rds" )
```


```{r}
fit.nw.unsup <- readRDS("../output/method-train-classifiers.Rmd/fit.nw.unsup.rds")
...
mean(sapply(1:5, function(i) mean(xy_time[[i]]$diff_time_nw)))/(2*pi)/(pi/(2*pi))
mean(sapply(1:5, function(i) mean(xy_time[[i]]$diff_time_trend2)))/(2*pi)/(pi/(2*pi))
mean(sapply(1:5, function(i) mean(xy_time[[i]]$diff_time_bspline)))/(2*pi)/(pi/(2*pi))
mean(sapply(1:5, function(i) mean(xy_time[[i]]$diff_time_loess)))/(2*pi)/(pi/(2*pi))

mean(fit.nw.unsup$diff_time)/(2*pi)/(pi/(2*pi))
mean(fit.trend2.unsup$diff_time)/(2*pi)/(pi/(2*pi))
mean(fit.bspline.unsup$diff_time)/(2*pi)/(pi/(2*pi))
mean(fit.loess.unsup$diff_time)/(2*pi)/(pi/(2*pi))
```
...
saveRDS(fit.trend2.unsup.split, 
        "../output/method-train-classifiers.Rmd/fit.trend2.unsup.split.rds")
```


```{r}
fit.nw.unsup.split <- readRDS("../output/method-train-classifiers.Rmd/fit.nw.unsup.split.rds")
...
save(pve.split.nw, pve.split.trend2, pve.split.bspline, pve.split.loess,
     file="../output/method-train-classifiers.Rmd/pve.methods.unsupervised.split.rda")
```


```{r}
load(file="../output/method-train-classifiers.Rmd/pve.methods.unsupervised.split.rda")
c(mean(pve.split.nw),
