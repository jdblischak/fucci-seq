...
sd(sapply(diff_time, mean)/2/pi)


cors_fl <- sapply(1:5, function(i) rFLIndTestRand(fits[[i]]$theta_test, 
                                              fits[[i]]$fit.test$cell_times_est, NR=10000) )
mean(cors_fl[1,])

cors_sp <- sapply(1:5, function(i) cor(fits[[i]]$theta_test, 
                                              fits[[i]]$theta_est_shift, method="spearman") )
mean(cors_sp)
...
## Re-fitting after removing PC outliers
...
plot(fits_sub[[1]]$theta_test,
     fits_sub[[1]]$fit.test$cell_times_est)
...
cors_fl <- sapply(1:5, function(i) rFLIndTestRand(fits_sub[[i]]$theta_test, 
                                              fits_sub[[i]]$fit.test$cell_times_est, NR=10000) )
mean(cors_fl[1,])

cors_sp <- sapply(1:5, function(i) cor(fits_sub[[i]]$theta_test, 
                                              fits_sub[[i]]$theta_est_shift, method="spearman") )
mean(cors_sp)
